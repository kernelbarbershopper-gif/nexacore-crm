// Initialize Data if not provided by main app
window.NexaData = window.NexaData || {};

if (!NexaData.conversations) {
    NexaData.conversations = [
        {
            id: 'c1',
            name: 'Ana Clara',
            platform: 'whatsapp',
            ...
            sentiment: 'positive',
            time: '10:45',
            lastMessage: 'Amei a nova funcionalidade! Como ativo?',
            status: 'online',
            dna: {
                ltv: 'R$ 4.500',
                churnRisk: 'Baixo (12%)',
                profile: 'Early Adopter',
                lastPurchase: 'Há 15 dias',
                tags: ['VIP', 'Tech', 'Engajada']
            },
            messages: [
                { id: 'm1', sender: 'agent', text: 'Olá Ana, tudo bem? Vi que você atualizou o app.', time: '10:40' },
                { id: 'm2', sender: 'contact', text: 'Oi! Tudo sim.', time: '10:44' },
                { id: 'm3', sender: 'contact', text: 'Amei a nova funcionalidade! Como ativo?', time: '10:45' }
            ]
        },
        {
            id: 'c2',
            ...
            sentiment: 'negative',
            time: 'Ontem',
            lastMessage: 'Meu pedido ainda não chegou, estou aguardando.',
            status: 'offline',
            dna: {
                ltv: 'R$ 800',
                churnRisk: 'Alto (78%)',
                profile: 'Cliente Impaciente',
                lastPurchase: 'Há 7 dias',
                tags: ['Atraso', 'Reclamação']
            },
            messages: [
                { id: 'm1', sender: 'contact', text: 'Fiz um pedido na semana passada e nada.', time: 'Ontem 14:00' },
                { id: 'm2', sender: 'agent', text: 'Olá Marcos, vamos verificar o status para você.', time: 'Ontem 14:15' },
                { id: 'm3', sender: 'contact', text: 'Meu pedido ainda não chegou, estou aguardando.', time: 'Ontem 15:30' }
            ]
        },
        {
            id: 'c3',
            ...
            sentiment: 'neutral',
            time: '09:15',
            lastMessage: 'Pode me enviar o catálogo em PDF?',
            status: 'online',
            dna: {
                ltv: 'R$ 1.200',
                churnRisk: 'Médio (45%)',
                ...
                tags: ['Atacado', 'Recorrente']
            },
            messages: [
                { id: 'm1', sender: 'contact', text: 'Bom dia, gostaria de ver as novidades.', time: '09:10' },
                { id: 'm2', sender: 'contact', text: 'Pode me enviar o catálogo em PDF?', time: '09:15' }
            ]
        }
    ];
}

// Removed mock NexaAI overrides to use ai-engine.js real implementation
// State
let activeConversationId = NexaData.conversations[0]?.id;

// DOM Elements
const contactListEl = document.getElementById('contactList');
const chatHeaderEl = document.getElementById('chatHeader');
const chatMessagesEl = document.getElementById('chatMessages');
const ghostWriterPanelEl = document.getElementById('ghostWriterPanel');
const dnaProfileEl = document.getElementById('dnaProfile');
    ...
    if (contact.dna) {
        dnaProfileEl.innerHTML = `
            <div class="dna-section">
                ...
                <div class="dna-value" style="color:var(--primary-400); font-weight:600; font-size:1.125rem">\${contact.dna.ltv}</div>
            </div>
            <div class="dna-section">
                <div class="dna-label">Risco de Churn</div>
                <div class="dna-value \${contact.dna.churnRisk.includes('Alto') ? 'text-danger' : 'text-success'}">\${contact.dna.churnRisk}</div>
            </div>
            <div class="dna-section">
                <div class="dna-label">Perfil de Compra</div>
                <div class="dna-value">\${contact.dna.profile}</div>
            </div>
            <div class="dna-section">
                <div class="dna-label">Última Compra</div>
                <div class="dna-value">\${contact.dna.lastPurchase}</div>
            </div>
            <div class="dna-section">
                <div class="dna-label">Tags</div>
                <div class="dna-tags">
                    \${contact.dna.tags.map(t => \`<span class="dna-tag">\${t}</span>\`).join('')}
                </div>
            </div>
        \`;
    }
