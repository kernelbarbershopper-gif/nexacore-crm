...
const NexaApp = {
  ...
  initHeader() {
    ...
    if (searchInput) {
      searchInput.addEventListener('focus', () => {
        searchInput.parentElement.style.width = '380px';
      });
      searchInput.addEventListener('blur', () => {
        searchInput.parentElement.style.width = '320px';
      });
    }

    // Update notification count
    const notifDot = document.querySelector('.notification-dot');
    const unread = NexaData.getUnreadCount();
    if (notifDot && unread === 0) {
  ...
  showToast(message, type = 'info', duration = 4000) {
    ...
    toast.innerHTML = `
      ...
      <button onclick="this.parentElement.remove()" class="btn-ghost btn-icon sm" style="opacity:0.5">✕</button>
    `;
  ...
  /* ── Animated Counter ── */
  animateCounter(element, target, duration = 1500, prefix = '', suffix = '') {
    const start = 0;
    ...
    function update(currentTime) {
      ...
      element.textContent = prefix + NexaData.formatNumber(current) + suffix;
  ...
  getPlatformColor(platform) {
    return platform === 'instagram' ? 'var(--accent-pink)' : 'var(--accent-green)';
  },

  getPlatformIcon(platform) {
    if (platform === 'instagram') {
      return `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg>`;
    }
  ...
  icons: {
    ...
    pipeline: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="3" width="6" height="18" rx="1"/><rect x="9" y="8" width="6" height="13" rx="1"/><rect x="17" y="13" width="6" height="8" rx="1"/></svg>',
    ...
    brain: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2a7 7 0 0 0-7 7c0 3 2 5.5 5 7v2h4v-2c3-1.5 5-4 5-7a7 7 0 0 0-7-7z"/><line x1="12" y1="22" x2="12" y2="18"/><line x1="9" y1="22" x2="15" y2="22"/></svg>',
